    ...
    <div className="bg-white border border-rose-200 rounded-2xl p-6 sm:p-7 shadow-xs flex flex-col gap-5" id="organisation-deletion-panel">
        ...
        <>
          <div className="flex flex-col gap-1 mx-auto w-full max-w-2xl">
            ...
            <p className="text-xs text-slate-500 leading-relaxed">
              ...
              and the registration email can never be used to register again. The organisation is suspended straight away and it can be restored
        ...
        <div className="flex flex-col gap-3 mx-auto w-full max-w-2xl">
          ...
          <div className="flex items-center gap-2">
            <button
              type="button"
              disabled={busy}
              onClick={() => run(() => api.requestOrganisationDeletion(tenantId, reason.trim() || undefined), () => setAskingReason(false))}
      ...
      {deletion?.nextAction === 'confirm_name' && (
        <div className="flex flex-col gap-3 mx-auto w-full max-w-2xl">
          ...
          <p className="text-xs text-slate-500 leading-relaxed">
            The organisation is suspended as soon as you confirm: its mailboxes stop working and its admins cannot change anything. You can restore it any time before the
      ...
      {deletion?.nextAction === 'initiate_otp' && (
        <div className="flex flex-col gap-3 mx-auto w-full max-w-2xl">
          ...
          <div className="flex items-center gap-2">
            {unlocked && (
              <button type="button" disabled={busy} onClick={() => run(() => api.initiateOrganisationOtp(tenantId))} className="btn btn-danger btn-sm">
      ...
      {deletion?.nextAction === 'generate_final_otp' && (
        <div className="flex flex-col gap-3 mx-auto w-full max-w-2xl">
          {!unlocked && deletion.nextActionAvailableAt ? (
            ...
            <p className="text-xs text-slate-600">You can now have the deletion code emailed to you.</p>
          )}
          <div className="flex items-center gap-2">
            {unlocked && (
              <button type="button" disabled={busy} onClick={() => run(() => api.generateOrganisationOtp(tenantId))} className="btn btn-danger btn-sm">
                Email me the code
              </button>
            )}
            {restoreButton}
      ...
      {deletion?.nextAction === 'verify_final_otp' && (
        <div className="flex flex-col gap-3 mx-auto w-full max-w-2xl">
          ...
          <p className="text-xs text-slate-500 leading-relaxed">
            The code is valid for {timings?.finalOtpMinutes ?? 10} minutes. Entering it permanently deletes {deletion.organisationName}.
          </p>
          <input
            id="deletion-otp"
            value={otpCode}
          ...
          <div className="flex flex-wrap items-center gap-2">
            <button
              ...
              onClick={() =>
                run(() => api.verifyOrganisationOtp(tenantId, otpCode), (d) => {
                  setOtpCode('');
            ...
            <button type="button" disabled={busy} onClick={() => run(() => api.generateOrganisationOtp(tenantId))} className="btn btn-secondary btn-sm">
              Send a new code
            </button>
            {restoreButton}
          </div>
