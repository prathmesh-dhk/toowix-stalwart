...
export const OrganisationDeletionPanel: React.FC<OrganisationDeletionPanelProps> = ({ tenantId, organisationName, onDeleted, onChanged }) => {
  ...
  const load = useCallback(async () => {
    try {
      setState(await api.getOrganisationDeletion(tenantId));
    } catch (err: any) {
  ...
  const cancel = async () => {
    ...
    setError(null);
    try {
      await api.cancelOrganisationDeletion(tenantId);
  ...
  const steps: { key: string; title: string; detail: string }[] = deletion
    ? [
        ...
        { key: 'verify', title: 'Verify the final code', detail: `A 6-digit code is emailed to your own address and is valid for ${timings?.finalOtpMinutes ?? 10} minutes.` },
        { key: 'delete', title: 'Permanent deletion', detail: 'Deletes everything. The registration email is blocked from registering again.' },
      ]
    : [];
          ...
          {!showStartForm ? (
            ...
            <div className="flex flex-col gap-3 max-w-2xl p-4 bg-rose-50/50 border border-rose-200/80 rounded-xl">
              <label htmlFor="deletion-reason" className="text-xs font-semibold text-rose-900">
                Reason for deletion <span className="font-normal text-rose-700">(optional, kept in the audit record)</span>
              </label>
              <textarea
                id="deletion-reason"
                value={reason}
              ...
              <p className="text-xs text-rose-700 leading-relaxed">
                Starting now suspends the organisation immediately: its mailboxes stop working and its admins cannot change anything until the deletion is cancelled.
              </p>
              <div className="flex items-center gap-2">
                <button
                  type="button"
                  disabled={busy}
                  onClick={() => run(() => api.requestOrganisationDeletion(tenantId, reason.trim() || undefined), () => setShowStartForm(false))}
      ...
      {deletion && (
        <>
          <div className="flex items-start gap-2 p-3 bg-amber-50 border border-amber-200 rounded-xl text-xs text-amber-900">
            ...
            <span>
              <strong>{deletion.organisationName}</strong> is suspended and scheduled for deletion. The organisation cannot be changed until the deletion is cancelled or completes.
            </span>
          </div>
          ...
          {deletion.nextAction === 'confirm_name' && unlocked && (
            <div className="flex flex-col gap-2 max-w-md">
              ...
              <button
                ...
                onClick={() => run(() => api.confirmOrganisationName(tenantId, typedName))}
          ...
          {deletion.nextAction === 'initiate_otp' && unlocked && (
            <button
              type="button"
              disabled={busy}
              onClick={() => run(() => api.initiateOrganisationOtp(tenantId))}
              className="btn btn-danger btn-sm self-start"
            >
          ...
          {deletion.nextAction === 'generate_final_otp' && unlocked && (
            <button
              type="button"
              disabled={busy}
              onClick={() => run(() => api.generateOrganisationOtp(tenantId))}
              className="btn btn-danger btn-sm self-start"
            >
            ...
            <div className="flex flex-col gap-2 max-w-xs">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  onClick={() => run(() => api.verifyOrganisationOtp(tenantId, otpCode), () => setOtpCode(''))}
                  className="btn btn-danger btn-sm"
                >
                  Verify code
                </button>
          ...
          {deletion.nextAction === 'final_confirmation' && (
            <div className="flex flex-col gap-2 max-w-xl p-4 bg-rose-50/50 border border-rose-200/80 rounded-xl">
              ...
              <p className="text-xs text-rose-700 leading-relaxed">
                Permanently delete <strong>{deletion.organisationName}</strong>, all of its domains, mailboxes and administrators.
              </p>
              <button
                type="button"
                disabled={busy}
                onClick={() => run(() => api.completeOrganisationDeletion(tenantId), onDeleted)}
