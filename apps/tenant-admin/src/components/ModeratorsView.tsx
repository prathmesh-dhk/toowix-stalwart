        ...
        <div className="p-3.5 border-b border-slate-100 bg-slate-50/50 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div className="relative flex-1 max-w-sm w-full">
            ...
            <input
              ...
              className="w-full pl-8 pr-3 py-2 sm:py-1.5 bg-white border border-slate-200 rounded-md text-base sm:text-xs text-slate-800 focus:outline-hidden focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500"
            />
          </div>
                ...
                >
                  <div className="flex items-start justify-between gap-2.5">
                    <div className="min-w-0">
                      <div className="font-semibold text-slate-900 text-sm truncate">{m.email}</div>
                      <div className="text-[11px] text-slate-400">Moderator Account</div>
                    </div>
                    <span
                  ...
                  <div className="space-y-1.5 pt-1 border-t border-slate-100">
                    <div className="text-[10px] uppercase font-semibold text-slate-400 tracking-wider">Scoped Domains</div>
                    {m.scopedDomainIds.length === 0 ? (
                      <span className="text-slate-400 italic text-xs">No domains assigned</span>
                    ) : (
                      <div className="flex flex-wrap gap-1">
                        {m.scopedDomainIds.map((id) => (
                          <span
                    ...
                    <button
                      onClick={() => openEditScope(m)}
                      className="text-xs font-semibold text-indigo-600 hover:text-indigo-700 cursor-pointer pt-0.5 inline-block"
                    >
                      Edit scope
                    </button>
                  </div>
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-2 text-xs text-slate-500 pt-1 border-t border-slate-100">
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        onClick={() => handleToggleStatus(m)}
                        disabled={togglingId === m.id}
                        className="min-h-[44px] px-3.5 py-2 text-slate-700 hover:text-slate-900 bg-slate-100 hover:bg-slate-200 rounded-lg font-medium text-xs transition-colors inline-flex items-center gap-1.5 cursor-pointer disabled:opacity-50"
                      >
                        <Ban size={13} />
                        <span>{m.status === 'active' ? 'Disable' : 'Enable'}</span>
                      </button>
                      <button
                        onClick={() => setDeletingTarget(m)}
                        className="min-h-[44px] px-3.5 py-2 text-rose-700 bg-rose-50 hover:bg-rose-100 border border-rose-200 rounded-lg font-medium text-xs transition-colors inline-flex items-center gap-1.5 cursor-pointer"
                      >
                        <Trash2 size={13} />
                        <span>Remove</span>
                      </button>
