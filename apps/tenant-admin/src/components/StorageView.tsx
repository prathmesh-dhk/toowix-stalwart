    ...
    <div className="flex flex-col gap-6 w-full max-w-6xl mx-auto py-2">
      ...
      {error && (
        <div className="p-4 bg-red-50 border border-red-200 rounded-xl text-xs text-red-700 flex items-center justify-between">
          ...
          <button
            onClick={() => fetchStorageData(true)}
            className="font-medium underline hover:text-red-900 cursor-pointer ml-4 min-h-[44px] sm:min-h-0 flex items-center"
          >
            Retry
          </button>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div className="bg-white border border-slate-200/90 rounded-2xl p-4 sm:p-5 shadow-xs flex flex-col gap-3">
          ...
          <div className="flex items-baseline gap-2">
            <span className="text-xl sm:text-2xl font-bold text-slate-900 tracking-tight" data-testid="total-storage-val">
        ...
        <div className="bg-white border border-slate-200/90 rounded-2xl p-4 sm:p-5 shadow-xs flex flex-col gap-3">
          ...
          <div className="flex items-baseline gap-2">
            <span className="text-xl sm:text-2xl font-bold text-slate-900 tracking-tight" data-testid="mailbox-count-val">
        ...
        <div className="bg-white border border-slate-200/90 rounded-2xl p-4 sm:p-5 shadow-xs flex flex-col gap-3">
          ...
          <div className="flex items-baseline gap-2">
            <span className="text-xl sm:text-2xl font-bold text-slate-900 tracking-tight" data-testid="avg-storage-val">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white border border-slate-200 rounded-2xl p-3 sm:px-4 shadow-xs">
        <div className="relative flex-1 max-w-md">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2 sm:py-1.5 text-base sm:text-xs bg-slate-50 border border-slate-200 rounded-xl text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all min-h-[44px] sm:min-h-0"
            id="storage-search-input"
          />
        </div>

        <div className="flex flex-wrap items-center gap-2 self-start sm:self-auto text-xs text-slate-500">
          <span>Sort by:</span>
          <button
            onClick={() => handleSort('storageBytes')}
            className={`min-h-[36px] sm:min-h-0 px-2.5 py-1 rounded-lg border font-medium transition-colors cursor-pointer flex items-center gap-1.5 ${
          ...
          <div>
            ...
            <div className="block md:hidden divide-y divide-slate-100">
              {filteredAndSortedMailboxes.map((mb) => (
                <div key={mb.id} className="p-4 flex flex-col gap-3">
                  ...
                  <div className="flex flex-col gap-1.5 bg-slate-50 p-3 rounded-xl border border-slate-100">
                    <div className="flex items-baseline justify-between text-xs">
                      ...
                      <span className="text-[11px] text-slate-500 font-mono">
                        {mb.percentage}% of domain storage
                      </span>
                    </div>
                  ...
                  <div className="flex items-center justify-between text-[11px] text-slate-400 px-0.5">
                    <span className="inline-flex items-center px-2 py-0.5 rounded-md font-medium bg-slate-100 text-slate-700 border border-slate-200/60">
                      {mb.domainName}
                    </span>
