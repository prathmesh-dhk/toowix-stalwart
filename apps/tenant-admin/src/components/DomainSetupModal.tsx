...
export const DomainSetupModal: React.FC<DomainSetupModalProps> = ({
  ...
  // Success state with created domain & DNS records
  const [createdDomain, setCreatedDomain] = useState<DomainItem | null>(null);
  const [dnsRecords, setDnsRecords] = useState<GeneratedDnsRecord[]>([]);
            ...
            <div>
              <div className="flex items-center gap-2">
                <h3 className="text-base font-semibold text-slate-900">
                  {step === 'form' ? 'Add New Domain' : 'Domain DNS Configuration'}
                </h3>
              ...
              <p className="text-xs text-slate-500 mt-0.5">
                {step === 'form'
                  ...
                  : `Publish these authoritative records for ${createdDomain?.domainName} to complete setup.`}
              </p>
            </div>
          </div>
          <button
            type="button"
            onClick={step === 'dns' ? handleFinish : onClose}
            ...
            <div className="flex items-center justify-end gap-3 pt-3 border-t border-slate-100">
              ...
              >
                {loading ? (
                  ...
                  <>
                    <span>Create & Get DNS Records</span>
        ...
        {step === 'dns' && (
          <div className="flex flex-col gap-4 animate-in fade-in">
              ...
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <span className="text-xs font-semibold text-emerald-950 block">
                    Domain &quot;{createdDomain?.domainName}&quot; Provisioned!
                  </span>
                  <span className="text-[11px] text-emerald-700">
                    Allocated capacity: {createdDomain?.mailboxLimit} mailboxes.
                  </span>
                </div>
              </div>
            </div>
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs font-medium text-slate-800">{rec.desc}</span>
                      </div>
                    ...
                    <div className="flex items-center gap-2 bg-white p-2 rounded-lg border border-slate-200/80">
                      <span className="flex-1 font-mono text-[11px] text-slate-700 break-all select-all">
                        {rec.target}
                      </span>
                      <button
                        type="button"
                        onClick={() => handleCopy(rec.target, key)}
                        className="px-2 py-1 rounded-md border border-slate-200 text-[11px] font-medium text-slate-600 hover:bg-slate-50 flex items-center gap-1 shrink-0 cursor-pointer shadow-2xs"
            ...
            <div className="pt-3 border-t border-slate-100 flex items-center justify-end">
              ...
              >
                <span>Complete Setup & Switch to Domain</span>
                <Check className="w-4 h-4" />
              </button>
            </div>
          </div>
