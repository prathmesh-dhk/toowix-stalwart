...
/**
 * The deletion timeline. Exported so tests (and the UI, via the API) never hard-code them.
 * Flow: reason -> exact name (org suspended) -> 7 days -> request OTP -> 24h -> code sent -> code entered -> deleted.
 */
export const DELETION_TIMINGS = {
  /** Wait between confirming the name (which suspends the org) and being allowed to request the OTP. */
  suspensionMs: 7 * DAY_MS,
  /** Wait between requesting the OTP and the code being sent. */
...
export interface DeletionView {
  ...
  initiatedAt: Date;
  /** null until the name is confirmed (which is what suspends the organisation). */
...
function nextActionFor(deletion: IOrganisationDeletion): { action: NextAction; availableAt: Date | null } {
  switch (deletion.stage) {
    case 'requested':
      return { action: 'confirm_name', availableAt: null };
    case 'name_confirmed':
      return { action: 'initiate_otp', availableAt: deletion.suspensionEndsAt };
    case 'otp_initiated':
...
// ---------------------------------------------------------------------------
// step 1 — ask to delete (with a reason). Nothing is suspended yet.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// step 2 — exact organisation name: this is what suspends the organisation
// ---------------------------------------------------------------------------
...
export async function confirmOrganisationName(input: StageInput & { organisationName: string }): Promise<DeletionView> {
  ...
  deletion.suspensionEndsAt = new Date(now.getTime() + DELETION_TIMINGS.suspensionMs);
  await record(deletion, { stage: 'name_confirmed', action: 'name_confirmed', success: true }, actor, context, now);
  await deletion.save();

  // The organisation is suspended from this moment until it is restored or deleted.
  const tenant = await TenantModel.findById(tenantId);
  if (tenant) {
...
export async function getActiveDeletion(input: Pick<StageInput, 'tenantId' | 'actor'>): Promise<DeletionView | null> {
  ...
  return deletion ? toDeletionView(deletion) : null;
}
...
// ---------------------------------------------------------------------------
// step 3 — request the OTP (after the 7-day suspension)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// step 4 — the code is sent 24h later; entering it deletes the organisation
// ---------------------------------------------------------------------------
...
export async function verifyFinalOtp(input: StageInput & { code: string }): Promise<DeletionView> {
  ...
  // A correct code *is* the final confirmation: record it, then delete straight away.
...
// ---------------------------------------------------------------------------
// permanent deletion (runs when the correct code is entered)
// ---------------------------------------------------------------------------
...
async function finaliseDeletion(
  ...
  try {
    ...
    // Not deleted after all: the code is still valid until it expires, so entering it again retries.
    deletion.otpVerification = 'pending';
    deletion.otpVerifiedAt = null;
    return failStep(
      deletion,
      'final_confirmation',
      ...
      context,
      now,
      new OrganisationDeletionError(`Deletion failed part-way (${err.message}). Enter the code again to retry.`, 'PURGE_FAILED', 500)
    );
  }
